...
<script lang="ts" setup>
  ...
  const classes = computed(() => {
    // 当前组件为激活状态, 显示实线
    if (activeComponentId.value === props.config.id) return 'outline-(1 solid blue-6 offset--1)';
    // 当前组件为拖拽状态, 显示虚线
    if (dragComponentId.value === props.config.id) return 'outline-(1 dashed blue-6 offset--1)';
    // 没有组件处于拖拽状态时, 鼠标悬停显示虚线
    if (!dragComponentId.value) return '[&:hover]-(outline-(1 dashed blue-6 offset--1))';
    return '';
  });
