...
describe('useFileReader', () => {
  ...
  it('只传入第一个参数时, 创建一个 DataURL 文件读取', async () => {
  ...
  it('传入第二个参数为 DataURL 时, 创建一个 DataURL 文件读取', async () => {
  ...
  it('传入第二个参数为 Text 时, 创建一个 Text 文件读取', async () => {
  ...
  it('传入第二个参数为 ArrayBuffer 时, 创建一个 ArrayBuffer 文件读取', async () => {
  ...
  it('传入第二个参数为非指定参数时, 则会创建一个 DataURL 文件读取', async () => {
  ...
  it('第一个参数支持传入 MaybeRefOrGetter 类型对象', async () => {
    // Ref
    {
  ...
  it('第一个参数为 ref 且初始有值时, 会立即进行文件读取', async () => {
    const file = ref(textBlob);
  ...
  it('第一个参数为 ref 且初始无值时, 不会进行文件读取, 在写入值后, 会立即进行文件读取', async () => {
    const file = ref<Blob>();
  ...
  it('当前文件读取完成后, 修改文件后, 会重新进行文件读取', async () => {
    const file = ref(textBlob);
  ...
  it('当前文件读取中, 修改文件后, 会中断当前文件读取, 并重新进行文件读取', async () => {
    const file = ref(textBlob);
  ...
  it('读取失败时, 返回的 error 中包含错误信息', async () => {
    const file: any = ref();
    const { result, error, isReading } = useFileReader(file);
