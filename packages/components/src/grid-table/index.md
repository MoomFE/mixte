...
| fixedRowHeight<br><el-tag>v3.6.0</el-tag> | 启用虚拟列表时的固定行高度<br>&nbsp;- <small>设置后, 将不再动态计算行高, 提升渲染性能, 和 `estimatedRowHeight` 互斥</small><br>&nbsp;- <small>设置后, 将移除单元格上下内边距</small> | ``` number \| `${number}` ``` | - |
